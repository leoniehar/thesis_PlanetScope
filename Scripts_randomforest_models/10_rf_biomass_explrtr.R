...
## difference here: Estimation variable (biomass instead interpolated biomasss values)

#### Random Forest model for biomass with split per Explrtr
...
#### split data ####
info_variant = 'perExplrtr'
#unique(all$explrtr)

SCH <- all[all$explrtr == "SCH",]
HAI <- all[all$explrtr == "HAI",]
ALB <- all[all$explrtr == "ALB",]


ex = list(ALB, HAI, SCH)
ex_name = c("ALB","HAI","SCH") 
a = 1:length(ex)
