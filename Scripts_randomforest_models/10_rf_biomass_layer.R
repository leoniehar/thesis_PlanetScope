...
## difference here: 
## Estimation variable (biomass instead interpolated biomasss values)
## split per mosaic instead of Explrtr

#### Random forest model with split per mosaic
...
## outliers set to NAs
dataset['biomass_noout'] = dataset[est_var]

dataset <-  dataset%>% mutate(biomass_noout = replace(biomass_noout, biomass_noout < result[["stats"]][1], NA))
dataset <-  dataset%>% mutate(biomass_noout = replace(biomass_noout, biomass_noout > result[["stats"]][5], NA))

## plot with ggplot2 
#########mit cowplot als alle in einem mit einheitlichen layout??
box_out = ggplot(dataset, aes(y = biomass, x = ex_name[i])) + 
...
ggsave(filename= paste(ex_name[i],est_var,"boxplot_outliers.png", sep='_'), plot = last_plot() ,height= 6, width = 8)

## rename columns 
dataset <- dataset %>% 
  rename(
    #syntax: newname = oldname
    biomass_with = biomass,
    biomass = biomass_noout
  )
## delete column that includes outliers
dataset <- subset (dataset, select = -biomass_with)
...
ggsave(filename= paste(ex_name[i],est_var,"mtry.png", sep='_'), plot = last_plot(),height= 6, width = 8)
...
ggsave(filename= paste(ex_name[i],est_var,"var_importance.png", sep = '_'), plot = last_plot(),height= 6, width = 8)
...
scatter = ggplot(testing_set,aes(x=biomass, y = predicted))+
  ...
  xlab('biomass (ground truth) [g/m^2]')+
  ...
  annotate(geom="text", x=max(testing_set$biomass)/5, y=9*(max(predicted)/10), label=paste("n = ",length(predicted)),
           color="black")
scatter
...
# RMSE
rmse = sqrt(mean((testing_set$biomass - predicted)^2))

# relative (normalized) RMSE
nrmse = (rmse/mean(info[[ex_name[i]]][["validation"]][["testing_set"]]))*100

# R^2
r2 = cor(testing_set$biomass, predicted) ^ 2
