...
## difference here: Estimation variable (RPM instead interpolated biomasss values)

#### Random forest model for RPM and split per explrtr
...
## outliers set to NAs
dataset['RPM_noout'] = dataset[est_var]

dataset <-  dataset%>% mutate(RPM_noout = replace(RPM_noout, RPM_noout < result[["stats"]][1], NA))
dataset <-  dataset%>% mutate(RPM_noout = replace(RPM_noout, RPM_noout > result[["stats"]][5], NA))

## plot with ggplot2 
#########mit cowplot als alle in einem mit einheitlichen layout??
box_out = ggplot(dataset, aes(y = RPM, x = ex_name[i])) + 
...
dataset <- subset (dataset, select = -RPM_with)
...
scatter = ggplot(testing_set,aes(x=RPM, y = predicted))+
  ...
  xlab('RPM (ground truth) [cm]')+
  ylab('predicted RPM [cm]')+
  geom_abline(intercept = 0, slope = 1, col= 'blue')+
  annotate(geom="text", x=range(testing_set$RPM)/5, y=9*(range(predicted)/10), label=paste("n = ",length(predicted)),
           color="black")
ggsave(filename= paste(ex_name[i],info_variant,est_var,"scatter_predicted_vali.png", sep='_'), plot = last_plot(),height= 6, width = 8)


# RMSE
rmse = sqrt(mean((testing_set$RPM - predicted)^2))

# relative (normalized) RMSE
nrmse = (rmse/mean(info[[ex_name[i]]][["validation"]][["testing_set"]]))*100

# R^2
r2 = cor(testing_set$RPM, predicted) ^ 2


info[[ex_name[i]]][['validation']][['predicted']] = predicted
info[[ex_name[i]]][['validation']][['validation']] = testing_set$RPM
